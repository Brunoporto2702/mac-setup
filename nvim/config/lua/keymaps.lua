...
-- Diagnósticos
map("n", "<leader>D",  function() vim.diagnostic.open_float({ scope = "line" })   end, { desc = "Diagnóstico da linha" })
map("n", "<leader>d",  function() vim.diagnostic.open_float({ scope = "cursor" }) end, { desc = "Diagnóstico do cursor" })
map("n", "<leader>nd", function() vim.diagnostic.jump({ count = 1 })  end, { desc = "Próximo diagnóstico" })
map("n", "<leader>pd", function() vim.diagnostic.jump({ count = -1 }) end, { desc = "Diagnóstico anterior" })
map("n", "<leader>dl", vim.diagnostic.open_float, { desc = "Mostrar diagnóstico da linha" })
map("n", "<leader>q",  function() vim.diagnostic.setloclist({ open = true }) end, { desc = "Lista de diagnósticos" })
